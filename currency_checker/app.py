...
def compare_color_histograms(image1, image2, bins=256):
    ...
    # Compare histograms using correlation and average the results
    similarity = sum(cv2.compareHist(hist1[i], hist2[i], cv2.HISTCMP_CORREL) for i in range(3)) / 3
    return hist1, hist2, similarity
...
def compare_with_standard(standard, test, color_threshold=0.85, edge_threshold=0.80):
    ...
    hist1, hist2, color_similarity = compare_color_histograms(standard, test)
...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            message = "The currency is genuine." if color_similarity > 0.85 and edge_similarity > 0.80 else "The currency is fake."
        except Exception as e:
            flash(f"An error occurred: {e}")
            return redirect(request.url)

        flash(message)
        return render_template('index.html', color_similarity=color_similarity, edge_similarity=edge_similarity,
